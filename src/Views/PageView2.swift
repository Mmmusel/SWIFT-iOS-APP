   ...
   // }
    ...
    VStack(spacing: 10) {
        ...
        HStack{Button(
            ...
            Spacer()
            Text((self.userData.isManager) ? "发通知" : "写评价")
        ...
        VStack{
         ...
         HStack{
            Button(
             action: { if(self.userData.userName == "")
             { self.registerAlert=true }
             
             else { self.chooseStore = true } }, label: {
            ...
            .frame(height: 70)
                .sheet(isPresented: self.$chooseStore,content:{
                    ChooseView(stores: (self.userData.isManager) ? self.userData.stores.filter {$0.manager == self.userId} : self.userData.stores){ ss in
